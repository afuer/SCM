...
(function(window, $) {
    ...
    var _StreamTable = function(container, opts, data) {
        ...
        if (!this.view)
            $.error('Add view function in options.');

        if (this.$container.get(0).tagName == 'TABLE')
            this.$container = this.$container.find('tbody');
    ...
    _F.searchInData = function(text) {
        ...
        for (i; i < l; i++) {
            if (this.text_index[i].indexOf(t) != -1)
    ...
    _F.streamData = function(time) {
        ...
        _self.timer = timer_func(function() {
            _self.fetchData();
            if (!_self.opts.fetch_data_limit)
                clearTimeout(_self.timer);
        }, time);
    };
    ...
    _F.paginate = function(page) {
        ...
        if (page == this.current_page || page < 0 || page >= page_count)
            return;
        ...
        if (this.paging_opts.span <= page_count)
            this.renderPagination(page_count, this.current_page);
    ...
    _F.renderPagination = function(page_count, current_page) {
        ...
        if (page_count > span) {
            ...
            if (current_page > (center - 1))
                i = current_page - center;
            if (current_page < (page_count - center - 1))
                l = i + span;
        }
        ...
        if (page_count > this.paging_opts.span)
            links.push('<li><a href="#" class="last">Last</a></li>');
    ...
    _F.addPerPage = function() {
        ...
        if (per_page_select === false || typeof per_page_select == 'string')
            return;
    ...
    _F.renderByPerPage = function(per_page) {
        if (this.paging_opts.per_page == per_page)
            return;
