...
function loadGrid() {
    ...
    $('#dataGrid').datagrid({
        ...
        url: 'get.php',
        columns: [[
                {field: 'LEDGER_CODE', title: 'Ledger Code'},
                {field: 'LEDGER_NAME', title: 'Ledger Name'},
                {field: 'UNDER_NAME', title: 'Under Ledger'},
...
function Edit() {
    ...
    if (row) {
        ...
        $('#fm').form('load', row);
        url = 'save.php?mode=""&search_id=' + row.LEDGER_ACCOUNT_ID;
    }
}
function Save() {
//alert(22);
        ...
        $.messager.confirm('Confirm', 'Are you sure you want to remove this user?', function(r) {
            if (r) {
                $.post('save.php?mode=delete',
                        {search_id: row.LEDGER_ACCOUNT_ID},
                function(result) {
                    if (result.success) {
                        $('#dataGrid').datagrid('reload'); // reload the user data
                    } else {
