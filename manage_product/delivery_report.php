            ...
            <div title="Delevery Challan Create Divisiton" >  
                ...
                <form method="POST" action="">
                    <table width="100%" class="ui-state-default">
                        ...
                        WHERE so.OFFICE_TYPE_ID=1 and ch.status =0 AND '$ProcessDeptId' IN (so.PROCESS_DEPT_ID)
            ...
            <div title="Delevery Challan Create Branch">
                <form action="" method="POST"> 
                    <table width="100%" class="ui-state-default">
                        ...
                        from challan ch 
                        left join challan_item chi on chi.challanid = ch.challanid 
                        LEFT JOIN product p ON p.PRODUCT_ID=chi.productid
                        ...
                        WHERE ot.OFFICE_TYPE_ID=2 AND ch.status =0 AND p.PROCESS_DEPT_ID='$ProcessDeptId'
                        GROUP BY chi.challanid ORDER BY chi.challanid DESC";

                        $sql_branch = $db->query($sql_delever);
                        $sub_total = "";
                        while ($rec = fetch_object($sql_branch)) {
                            $sl++;
