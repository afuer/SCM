...
if (isSave()) {
    ...
    $RecommendBill = getParam("RecommendBill");

    $Module = "Expense Bill";

    if (isEmpty($invoice_date))
        $invoice_date = date("yyyy-m-d");
    ...
    $financialYear = findValue("select financial_year FROM financial_year WHERE '$today' BETWEEN start_date and end_date");

    $expanse_sql = "INSERT INTO fin_payment_approval_note(RECOMMEND_BILL,PAYMENT_NO, PURCHASE_ORDER_ID, PAYMENT_MODE_ID, INSTRUMENT_ID, BENEFICIARY_ID, PURPOSE, COMMENTS, `STATUS`, USER_LEVEL_ID, EXPENSE_DATE, INVOICE_NO, SECURITY_DEPOSIT, SECURITY_DEPOSIT_PERCENT, TAX_PERCENT, TAX_AMOUNT, VAT_PERCENT, VAT_AMOUNT, NET_PAY, CREATED_BY, CREATED_DATE,FINANCIAL_YEAR, PRENENT_LOCATION_ID) 
        values('$RecommendBill','$payentNo', '$poid', '$modeid', '$instrumentid', '$supplierid', '$purpose', '$comments', '1', '5', NOW(), '$invoice_no', '$securityDeposit', '$securityAmount', '$incomeTax', '$incomeTaxAmount', '$vat', '$vatAmount', '$netBill', '$employeeId', NOW(),'$financialYear', '$lineManagerId')";
    ...
    /* echo "<script>location.replace('expense_form.php?po_no=$poid');</script>";
      this was rijib via's code.

      new code by Zakir */
    echo "<script>location.replace('expense_details2.php?expence_id=$lastId&mode=view');</script>";
}
...
$vendor = find("SELECT SUM(IFNULL(NET_PAY,0))AS NET_PAY, SUM(IFNULL(VAT_AMOUNT,0)) AS VAT_AMOUNT,FINANCIAL_YEAR FROM fin_payment_approval_note WHERE BENEFICIARY_ID='$rec->supplier_id'");
?>
...
<script>
    ...
    function securityAmountChange() {
        var securityPercent = $('#securityDepositId').val(),
                workOrderAmount = $('#billId').val(), //previous data wo_amount
    ...
    $(document).ready(function() {
        ...
        $('#SuspenseAmountTrId').hide();
        $('#suspenseBillTrId').hide();
        $("#modeidID").change(function() {
            if ($(this).val() == 12)
                $('#suspenseBillTrId').show();
        });
        $("#suspenseBillId").change(function() {
            $('#SuspenseAmountTrId').show();
        });
    });
</script>
...
<div class="easyui-layout" style="margin: auto; height:1200px;">  
    <div Title='New Payment Approval Note' data-options="region:'center'" style="padding: 10px 10px; background-color:white; "> 
        <div style="border-style:solid;border-color:#FF3366;border-width:1px;"><br />
            <p style="font-size:18px;">
                Total Paid Amount to the <font style="color:#CC0033"><?php echo $rec->SUPPLIER_NAME; ?></font> <?php echo $vendor->FINANCIAL_YEAR; ?> financial year : <?php echo $vendor->NET_PAY; ?><br></p>
            <p style="font-size:18px;"> Total Vat Amount in this financial year: <?php echo $vendor->VAT_AMOUNT; ?><br>
            </p>
        </div><br />
        ...
        <form name="expanse_form" action="" class="formValidate" method="POST" enctype="multipart/form-data" autocomplete="off">
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        <a href="po_view.php?poid=<?php echo $rec->purchase_order_id; ?>" target=_blank><?php echo $rec->purchase_order_id; ?>
            ...
            <table class="table">
                <tr>
                    ...
                    <td><input id="invoice_no"  size="28" tabindex="11" name="invoice_no" class="required" onchange="ajaxLoader('ajax_invoice_check.php?val=' + this.value + '&supplierid=<?php echo $rec->supplier_id; ?>', 'ajax_invoice', '<left><img src=../images/ajaxLoader.gif></left>');" />
                        <div id="ajax_invoice"><input type="hidden" name="is_duplicate" value="0" /></div>  </td>
                </tr>
                <tr>
                ...
                <tr id="suspenseBillTrId">
                    <td align="right">Suspense Bill No: </td>
                    <td><input type="text"  size="28" name="suspenseBillNo" id="suspenseBillId" /></td>
                </tr>
                <tr id="SuspenseAmountTrId">
                    <td align="right">Suspense Bill Amount: </td>
                    <td><input type="text"  size="28" name="suspenseBillAmount" id="suspenseBillAmountId" value="hard coded value"/></td>
                </tr>

                <tr>
                    <td align="right">Recommend Bill: </td>
                    <td><input type="text"  size="28" name="RecommendBill" id="billId" value="<?php ?>" /></td>
                </tr>
                ...
                <tr>
                    ...
                    <td> <input type="text"  size="28" name="incomeTax" id="incomeTaxId" onchange="incomeTaxAmountChange();"  value="" /> Amount: <input type="text" name="incomeTaxAmount" id="incomeTaxAmountId" value="0" /></td>
                </tr>
            ...
            <button class='button' type='submit' value='save' name='save' >Submit</button>
