...
$CapexOpexType = rs2array(query("SELECT PRODUCT_GROUP_ID, PRODUCT_GROUP_NAME FROM product_group"));
$year = getParam('year');
...
<form method="POST" class="form">
    <table>
        <tr>
            ...
            <td><?php comboBox('CapexOpex', $CapexOpexType, $CapexOpex, FALSE); ?></td>
            ...
            <?php while ($Row = fetch_object($CCResult)) { ?>
                <tr>
                    ...
                    while ($HeadRowNew = fetch_object($Head1)) {
                        $ProcureSQL = "SELECT amount FROM delegation_authority WHERE gl_account_id='$Row->GL_ACCOUNT_ID' AND designation_id='$HeadRowNew->DESIGNATION_ID' AND year='$year' 
                                AND op_cap_type='$CapexOpex' AND EXPENSE_TYPE_ID='1'";
                        $ProcureAmount = findValue($ProcureSQL);
                        $NonProcureAmount = findValue("SELECT amount FROM delegation_authority WHERE gl_account_id='$Row->GL_ACCOUNT_ID' AND designation_id='$HeadRowNew->DESIGNATION_ID' AND year='$year' 
                                AND op_cap_type='$CapexOpex' AND EXPENSE_TYPE_ID='2'");
                        ?>
                        <td>
                            <input size="8" type="text"  designation="<?php echo $HeadRowNew->DESIGNATION_ID; ?>" account="<?php echo $Row->GL_ACCOUNT_ID; ?>" proc_type="1" name="txtbx[<?php echo $Row->GL_ACCOUNT_ID; ?>][<?php echo $HeadRowNew->DESIGNATION_ID; ?>]" class="textBox" value="<?php echo $ProcureAmount; ?>" style="margin-bottom: 2px;"/>
                            <input size="8" type="text"  designation="<?php echo $HeadRowNew->DESIGNATION_ID; ?>" account="<?php echo $Row->GL_ACCOUNT_ID; ?>"  proc_type="2" name="txtbx_non_proc[<?php echo $Row->GL_ACCOUNT_ID; ?>][<?php echo $HeadRowNew->DESIGNATION_ID; ?>]" class="textBox non_procure" value="<?php echo $NonProcureAmount; ?>" />
                        </td>
                        <?php
                    }
                    ?>
