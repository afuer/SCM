...
function loadGrid() {
    ...
    $('#dataGrid').datagrid({
        ...
        pagePosition: 'pos',
        idField: objectId,
        url: 'get_workflow_details.php',
        columns: [[
        { field: 'WORKFLOW_NAME', title: 'Work Flow Group' },
        { field: 'BRANCH_DEPT_NAME', title: 'Bran/Dept' },
        { field: 'DESIGNATION_NAME', title: 'Designation' },
        { field: 'DIVISION_NAME', title: 'Division' },
        { field: 'OFFICE_NAME', title: 'Office Type' },
        { field: 'WORKFLOW_PROCESS_NAME', title: 'Work Flow Process' }
...
function AddNew() {
    ...
    $('#fm').form('clear');
    url = 'save_workflow.php?mode=new';
}
function Edit() {
    ...
    if (row) {
        ...
        $('#fm').form('load', row);
        url = 'save_workflow.php?mode=""&search_id=' + row.WORKFLOW_GROUP_ID;
    }
}
function Save() {
    //alert(22);
...
function Remove() {
    ...
    if (row) {
        $.messager.confirm('Confirm', 'Are you sure you want to remove this user?', function(r) {
            if (r) {
                $.post('save_workflow.php?mode=delete',
                {
                    search_id: row.WORKFLOW_GROUP_ID
                },
                function(result) {
                    if (result.success) {
                        $('#dataGrid').datagrid('reload'); // reload the user data
