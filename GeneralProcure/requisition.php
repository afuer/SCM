...
class requisition extends DbManager {
    ...
    public function save($dto) {
        ...
        $lineManager = $manager->GetLineManager($dto->EmployeeId);

        $requisition_sql = "INSERT INTO requisition (REQUISITION_ID, REQUISITION_NO, PRESENT_LOCATION_ID, CREATED_BY, REQUISITION_DATE, SPECIFICATION, JUSTIFICATION, REMARK, PROCESS_DEPT_ID, REQUISITION_TYPE_ID, BRANCH_DEPT_ID, OFFICE_TYPE_ID, CREATED_DATE)
        VALUES('$maxRequisitionId','$requisition_no', '$lineManager->LINE_MANAGER_ID', '$dto->userName', NOW(),'$dto->specification','$dto->justification','$dto->remark', '$dto->processDeptId', '$dto->requisitionTypeId', '$dto->BranchDeptId', '$dto->OfficeType', NOW())";

        $result = $this->sql($requisition_sql);

        SaveUploadFile($maxRequisitionId, 'requisition', $dto->attachTitle, $dto->attachPath);
        ...
        foreach ($dto->productId as $key => $value) {
            ...
            $sqlDetails = "INSERT INTO requisition_details (REQUISITION_DETAILS_ID, REQUISITION_ID, PRODUCT_ID, QTY, UNIT_PRICE, USER_COMMENT) 
                VALUES('$maxRequisitionDetailsId', '$maxRequisitionId', '$productId[$key]', '$qty[$key]', '$lastPrice', '$remark[$key]')";
            $result = $this->sql($sqlDetails);
        }


        //$manager->InsertWorkFlowManager("$maxRequisitionId", "requisition", "$dto->EmployeeId");
    ...
    public function update($dto) {
        ...
        SaveUploadFile($dto->requisitionId, 'requisition', $dto->attachTitle, $dto->attachPath);
    ...
    public function RequisitionApproval($offset, $rows, $user_name, $UserLevelId) {
        ...
        WHERE rq.REQUISITION_STATUS_ID>0 AND rq.PRESENT_LOCATION_ID='$user_name'
    ...
    public function count($user_name, $dto = NULL) {
        ...
        $sql = "SELECT count(*) FROM requisition rq WHERE rq.CREATED_BY='$user_name' $res ";
        $rs = $this->query($sql);
        $row = fetch_row($rs);
