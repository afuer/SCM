...
if (!empty($btnchallan)) {
    ...
    $expire_date = getParam('expire_date');
    //print_r($challan_no);

    foreach ($done as $key => $value) {

        sql("insert stockmove (PRODUCT_ID, QTY, narrative, purchaseorderid, createdby, locationid, deliverd, STOCK_MOVE_DATE, salesorderid )
       ...
       $po_sql = "update purchaseorder_item_delivery set status=5,challanno='$challan_no[$value]', receive_date='$date',expire_date='$expire_date[$value]' where poid=$poid[$value] and orderid=$orderid[$value] and productid=$value";
    ...
    echo "<script>location.replace('goods_recieve.php')</script>";
}
...
$sql = "select pur.purchase_order_id,
        ...
        where pri.status in ($goods_status) 
...
<div class="easyui-layout" style="width:1100px; margin: auto; height:1000px;">  
    <div data-options="region:'center'" Title='Delivery Products' style="padding: 10px 10px; background-color:white; "> 
        ...
        <form action="" method="POST">
                ...
                <tbody>
                    ...
                    while ($rec = fetch_object($query)) {
                        ...
                        <tr>
                            ...
                            <?php  if ($rec->PROCESS_DEPT_ID =='12'){ ?>
                            <td><input type="text" name="expire_date[<?php echo $rec->productid; ?>]"  value="<?php echo $rec->currentDate; ?>" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser"  /></td>
                            <?php
                            }else{
                                echo "<td></td>";
                            }  ?>
            ...
            <input type="submit" name="btnchallan" value="Received">
