    ...
    <div data-options="region:'center'" Title='Add Supplier for this Comparative Statement' style="padding: 10px 10px; background-color:white; "> 
        ...
        <form method="post" action="">
            ...
            <table class="easyui-datagrid">
                ...
                <tbody>
                    <?php
                    $q = query("SELECT s.SUPPLIER_NAME, s.SUPPLIER_ID, 
                        (CASE WHEN sp.PRODUCT_ID IN ($product_list) THEN 1 ELSE 0 END) AS chk
                        FROM supplier s
                        LEFT JOIN supplier_price sp ON sp.SUPPLIER_ID=s.SUPPLIER_ID
                        #WHERE sp.PRODUCT_ID IN (1014)
                    ...
                    while ($row = fetch_object($q)) {
                        ...
                        <tr>
                            <td><?php echo++$sl; ?></td>
                            <td width="20">
                                <input type="hidden" name="supplier_ids[<?php echo $sl; ?>]" value="<?php echo $row->SUPPLIER_ID; ?>" />
